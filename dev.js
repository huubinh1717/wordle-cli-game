...
function ifContinue() {
    var input = uInput('wanna continue playing?(y/n): ').toLowerCase().trim();
    if (input === 'y') {
        game(wList);
        return;
    }
...
function game(wordList, word, numTurns) {
    ...
    console.log('\n');
    var guess = uInput('guess(legitimate, 5-letter): ')
        .toLowerCase()
        .trim();
    ...
    if (guess.length !== 5 ||
        ...
        console.log('invalid guess 😞😢, try inputting a meaningful 5-letter word ');
    ...
    if (turns > 0 && !done) {
        ...
        if (turns === 0) {
            console.log("\n\nAwww bad luck!\uD83D\uDE22\uD83D\uDE1E\uD83D\uDE3F it was: ".concat(chosenOne, "\n"));
            done = true;
            ifContinue();
            return 0;
        }
